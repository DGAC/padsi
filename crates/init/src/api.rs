...
async fn impl_post_procs(
    ...
    let pspec = ProcessSpec::new(
        start_req.args,
        &config.env,
        start_req.environ.as_ref(),
        start_req.capabilities,
        start_req.stdin_file,
        start_req.stdout_file,
        start_req.stderr_file,
        start_req.required,
        start_req.restart,
    );
...
async fn impl_get_procs(
    ...
    let config = config.lock().await;
    let list: Vec<ProcessInfo> = config
        .procs
        .values()
...
async fn impl_put_procs(
    ...
    match del_req.state.as_str() {
        ...
        any => {
            return Ok(json_error(
                StatusCode::BAD_REQUEST,
                &format!("invalid state '{}'", any),
            ));
        }
    }
}

async fn proc_status_get(
    config: SharedConfig,
    pid_str: &str,
) -> Result<Response<Full<Bytes>>, hyper::Error> {
    match pid_str.parse() {
                ...
                match v.as_ref() {
                    "auto-stop" => {
                        return Ok(json_response(
                            StatusCode::OK,
                            &PropValue::Bool(config.auto_stop),
                        ));
                    }
                    other => {
                        return Ok(json_error(
    ...
    match prop_req.name.as_str() {
        "auto-stop" => {
            ...
            return Ok(json_error(
                StatusCode::BAD_REQUEST,
                &format!(
                    "invalid value '{:?}' for the 'auto-stop' property",
                    prop_req.value
                ),
            ));
        }
