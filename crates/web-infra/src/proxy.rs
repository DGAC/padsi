...
use padsi::trace::{Instrument, Level, debug, error, info, span, trace, warn};

use crate::config::GlobalConfig;
use crate::misc::{
    empty, ensure_ip_address_present, full, get_endpoint_from_request, get_host_port, parse_ipv4,
    response_for_web_redir_resource,
};
...
async fn handle_request(
    ...
    if Method::CONNECT == req.method() {
        ...
        if let Some(addr) = get_host_port(req.uri()) {
                ...
                Ok(next_hop) => {
                    tokio::task::spawn(async move {
                        match hyper::upgrade::on(req).await {
                            Ok(upgraded) => {
                                if let Err(err) = tunnel(upgraded, &next_hop, &addr)
                                    ...
                                    .await
                                {
                                    span.in_scope(|| {
                                        ...
                                        match next_hop {
                                            Some(_s) => {
                                                warn!(up_proxy_blocked = ep.to_string(), "{}", msg)
                                            }
                                            None => {
                                                warn!(up_conn_blocked = ep.to_string(), "{}", msg)
                                            }
                                        }
                                    })
                                };
                            }
                            Err(e) => span.in_scope(|| warn!("upgrade error: {}", e)),
                        }
                    });
                ...
                Err(_err) => {
                    ...
                    if conf.web_redirector.is_some() {
                        ...
                        tokio::task::spawn(async move {
                            match hyper::upgrade::on(req).await {
                                Ok(upgraded) => {
                                    if let Err(err) = tunnel(upgraded, &None, &addr)
                                        .instrument(span.clone())
                                        .await
                                    {
                                        span.in_scope(|| {
                        ...
                        span.in_scope(|| trace!("Access denied, returning 403"));
                        let mut resp = Response::new(empty());
            ...
            span.in_scope(|| warn!("Invalid request URI: '{:?}'", req.uri()));
            let mut resp = Response::new(empty());
            *resp.status_mut() = StatusCode::BAD_REQUEST;
            Ok(resp)
        }
    } else {
        // for HTTP connections, forward request
        let proxy_conf = conf.web_proxy.as_ref().unwrap();
        match proxy_conf.get_hext_hop_addr(&ep) {
            Ok(next_hop) => {
                ...
                let addr = match &next_hop {
                    Some(p) => {
                        span.in_scope(|| trace!("Forwarding request to proxy {}", p));
                        p.clone()
                    ...
                    None => {
                        ...
                        let host = match req.uri().host() {
                            Some(h) => h,
                            None => {
                                span.in_scope(|| warn!("No host specified"));
                        ...
                        span.in_scope(|| trace!("Sending request to web server {}:{}", host, port));
                        format!("{}:{}", host, port)
                    }
                };
                    ...
                    Ok(stream) => {
                        ...
                        tokio::task::spawn(async move {
                            if let Err(err) = conn.await {
                                span2.in_scope(|| {
                                    warn!(
                                        up_conn_blocked = ep.to_string(),
                                        "HTTP connection failed: {:?}", err
                                    )
                                })
                            } else {
                                span2.in_scope(|| debug!("HTTP connection Ok"))
                            }
                        });
                        match next_hop {
                            Some(_) => {
                            ...
                            None => {
                                ...
                                match path_query.parse() {
                                    ...
                                    Err(err) => {
                                        ...
                                        msg = msg.replace('\n', "");
                                        error!(
                                            "Should not happen: got an invalid URI extracted from the request: {}",
                                            msg
                                        );
                                        let mut resp = Response::new(empty());
                                        *resp.status_mut() = StatusCode::BAD_REQUEST;
...
async fn tunnel(upgraded: Upgraded, remote_proxy: &Option<String>, addr: &str) -> Result<()> {
    ...
    let mut remote_server = match remote_proxy {
        Some(proxy_addr) => {
            ...
            span.in_scope(|| trace!("Tunnel via proxy {}", proxy_addr));
            ...
            if !response_str.contains(" 200 ") {
                span.in_scope(|| warn!("Proxy CONNECT failed: {response_str}"));
    ...
    let (from_client, from_server) =
        tokio::io::copy_bidirectional_with_sizes(&mut upgraded, &mut remote_server, 32768, 32768)
            .await?;

    // done
    span.in_scope(|| {
        info!(
            sent_bytes = from_client,
            recv_bytes = from_server,
            "tunnel closed"
        )
    });
