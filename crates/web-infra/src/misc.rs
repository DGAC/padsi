...
use hyper::{Method, Request, Response, StatusCode, Uri, body::Bytes};
use padsi::net::EndPoint;
use padsi::notifications::WebRedirNotification;
use padsi::trace::{error, info, warn};
...
pub async fn ensure_ip_address_present(ip: Ipv4Addr) -> Result<()> {
    ...
    while slept < 60000 {
        ...
        slept += 500;
        info!(
            "Waited a bit, try to see if there is a network inferface with the '{}' IP address",
            ip
        );
    }
    Err(anyhow!(
        "Could not find any network interface with the '{}' IP address",
        ip
    ))
}

pub async fn static_file_response(
    root_path: &Path,
    rel_filename: Option<&str>,
        ...
        Ok(contents) => {
            ...
            *resp.status_mut() = StatusCode::OK;
            if let Some(ct) = content_type {
                resp.headers_mut()
                    .append(CONTENT_TYPE, HeaderValue::from_str(ct).unwrap());
            }
            Ok(resp)
        }
        Err(err) => {
...
///
pub async fn response_for_web_redir_resource(
    ...
    if req.method() == Method::GET {
        ...
        if let Some(npath) = path.strip_prefix("/padsi-redir/") {
            ...
            if req.headers().get("referer").is_some()
                && let Some(browser) = detect_browser(req.headers())
            {
                if let Some(host) = get_host_from_headers(req)
                    && req.uri().path().len() < 100
                {
