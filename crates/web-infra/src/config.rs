...
use std::{env::current_exe, fs::File, io::{BufReader, Write}, path::{Path, PathBuf}, sync::{Arc, RwLock}};
use anyhow::{anyhow, Result};
use serde::{Deserialize, Serialize};
use tempfile::NamedTempFile;
use padsi::{net::{EndPoint,endpoint::Zone}, pki::CA};
use padsi::trace::{info};
use crate::cache::{SizedCache, CertificatesCache};
...
impl WebProxyConfig {
                ...
                for rule in target.rules.iter() {
                    for zone in rule.endpoint.zones() {
                        match zone {
                            ...
                            Zone::Name(dname) => pad.push_str(&format!("    if (host==\"{}\") return \"DIRECT\";\n",
                                                                     dname.without_trailing_dot())),
                            Zone::Network(n) => pad.push_str(&format!("    if (isInNet(host, \"{}\", \"{}\")) return \"DIRECT\";\n",
                                                                           n.ip().to_string(), n.mask().to_string())),
                            Zone::Pattern(p) => pad.push_str(&format!("    if (shExpMatch(url, \"{}\")) return \"DIRECT\";\n",
    ...
    pub fn get_hext_hop_addr(&self, req_ep:&EndPoint) -> Result<Option<String>> {
        ...
        if let Some(v)=cache.get(&ep_str) {
            info!(allowed_with=v, cached=true, allowed=req_ep.to_string(), "access allowed");
            return Ok(v.clone())
        }
        ...
        match (matching_endpoint, matching_target) {
            (Some(rule), Some(target)) => {
                match rule.action {
                    RuleAction::Allow => {
                        info!(allowed_with=target.remote_proxy, allowed=req_ep.to_string(), "access allowed");
                        cache.add(ep_str, target.remote_proxy.clone());
                        return Ok(target.remote_proxy.clone())
                    },
                    RuleAction::Deny => {
                        info!(blocked_with=target.remote_proxy, blocked=req_ep.to_string(), "access denied");
                        return Err(anyhow!("Access denied by '{:?}'", target.remote_proxy))
                    }
                }
            },
            _ => {
                info!(blocked=req_ep.to_string(), "Access not allowed by any rule");
...
impl std::fmt::Display for GlobalConfig {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "Config[proxy: {:?}, redirector: {:?}]",
