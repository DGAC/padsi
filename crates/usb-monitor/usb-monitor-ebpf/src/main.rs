...
use aya_ebpf::{macros::{tracepoint, map}, maps::{RingBuf, HashMap}, programs::TracePointContext};
use aya_ebpf::helpers::{bpf_probe_read_user_str_bytes, bpf_get_current_pid_tgid, bpf_get_current_uid_gid, bpf_get_current_comm};
...
fn handle_enter_open(ctx: TracePointContext) -> Result<u32, u32> {
    ...
    let _ = unsafe {
        bpf_probe_read_user_str_bytes(args.filename as *const u8, &mut filename_buf).map_err(|_| 0u32)?
    };
...
fn handle_enter_openat2(ctx: TracePointContext) -> Result<u32, u32> {
    let args: SysEnterOpenat2 = unsafe {
        match ctx.read_at(0) {
            Ok(r) => r,
            Err(err) => {
                error!(&ctx, "Failed to get args in enter_openat2(): error #{}", err);
                return Ok(0)
            }
        }
    };
    ...
    unsafe {
        if let Some(f)=TMP.get(tgid) {
            if args.ret >=0 {
                ...
                if let Err(err)=send_event_to_userspace(EventType::OPEN, uid, tgid, f, args.ret as u64) {
                    error!(&ctx, "Failed to send OPEN event to userspace: error #{}", err);
                }
            }
            if let Err(err)=TMP.remove(tgid) {
                error!(&ctx, "Failed to remove entry in TMP for PID {}: error #{}", tgid, err);
            }
        }
    }
    Ok(0)
    ...
    unsafe {
        ...
        if let Some(f)=ACTIVE.get(&akey) {
            ...
            if let Err(err)=send_event_to_userspace(EventType::CLOSE, uid, tgid, f, args.fd) {
                error!(&ctx, "Failed to send CLOSE event to userspace: error #{}", err);
            }
            ...
            match ACTIVE.remove(&akey) {
                Ok(_) => {
                    match PROC.get(tgid) {
                        Some(data) => {
                            ...
                            if !removed {
                                error!(&ctx, "Invalid PROC for PID {}, missing FD {}", tgid, args.fd)
                            }
                        }
                        None => {
                ...
                Err(err) => {
                    error!(&ctx, "Failed to remove entry in ACTIVE for PID {}: error #{}", tgid, err);
                }
            }
        }
    }
...
fn handle_process_exit(ctx: TracePointContext) -> Result<u32, u32> {
    ...
    if args.pid<0 {
        error!(&ctx, "sched_process_exit reports a negative PID {}", args.pid);
        return Ok(0)
    }
    unsafe {
        ...
        if let Some(data)=PROC.get(tgid) {
            ...
            for idx in 0..PROC_OPENED_FD_LEN {
                if data[idx]!=0 {
                    ...
                    match ACTIVE.get(&akey) {
                        Some (f) => {
                            if let Err(err)=send_event_to_userspace(EventType::CLOSE, uid, tgid, f, data[idx]) {
                                error!(&ctx, "Failed to send CLOSE event to userspace: error #{}", err);
                            }
                        ...
                        None => {
                            error!(&ctx, "Incoherence between ACTIVE and PROC for PID {} and FD {}", tgid, data[idx])
                        }
                    }
                }
            }
            ...
            if let Err(err)=PROC.remove(tgid) {
                error!(&ctx, "Failed to remove entry in PROC for PID {}: error #{}", tgid, err);
            }
        }
    }
...
fn send_event_to_userspace(ev_type:EventType, uid: u32, tgid: u64, path_buf: &[u8;BUF_PATH_LEN], fd:u64) -> Result<(), u32>{
