...
impl NetState {
    ...
    fn get_from_index(&mut self, index: u32) -> Result<String> {
        match self.ifaces.get(&index) {
            ...
            None => match self.update() {
                Ok(_) => match self.ifaces.get(&index) {
                    Some(name) => Ok(name.clone()),
...
fn main() -> Result<()> {
    ...
    if let Ok(v) = env::var("LOG_GROUP") {
        nflog_group = v
            .parse()
            .expect(&format!("Invalid LOG_GROUP value '{}'", v));
    }
    ...
    let mut group = match queue.bind_group(nflog_group) {
        ...
        Err(err) => {
            let msg = format!(
                "Failed to bind to group {}: {} (may be used by another program?)",
                nflog_group,
                err.to_string()
            );
            error!(msg);
    ...
    match protocol {
        6 => {
            ...
            if payload.len() >= transport_start + 4 {
                let src_port =
                    u16::from_be_bytes([payload[transport_start], payload[transport_start + 1]]);
...
fn parse_dns(payload: &[u8], transport_start: usize) -> Option<String> {
    ...
    // Parse DNS header
    let transaction_id = u16::from_be_bytes([payload[dns_start], payload[dns_start + 1]]);

    let flags = u16::from_be_bytes([payload[dns_start + 2], payload[dns_start + 3]]);
    ...
    let qdcount = u16::from_be_bytes([payload[dns_start + 4], payload[dns_start + 5]]);
    if qdcount > 0 {
