...
use aya_ebpf::helpers::{bpf_ktime_get_ns, bpf_probe_read_kernel, bpf_probe_read_kernel_str_bytes};
use aya_ebpf::{
    macros::{lsm, map},
...
use data_access_guard_common::{BUF_PATH_LEN, BlockedCall, Event, KEY_INIT_NETNS, KEY_PADSI_PID};
mod vmlinux;
...
fn may_block_from_path(
    ctx: &LsmContext,
    path_ptr: *const vmlinux::path,
) -> Result<(bool, Option<(*mut [u8; 4096], i64)>), i32> {
    ...
    if path_len < 0 {
        warn!(
            &ctx,
            "access blocked: path was too long or there was an error trying to get it"
        );
        return Err(-1);
    }
    ...
    unsafe {
        match (*path_buf).starts_with(PADSI_PROTECTED_USER)
            || (*path_buf).starts_with(PADSI_PROTECTED_LOG)
        {
    ...
    let exp_ppid = unsafe {
        match CONFIG.get(&KEY_PADSI_PID) {
            ...
            None => {
                warn!(
                    &ctx,
                    "access denied: CONFIG map does not contain the padsi's system service PID (code bug)"
                );
                return Err(-1);
            }
        }
    ...
    let init_net_ns = unsafe {
        match CONFIG.get(&KEY_INIT_NETNS) {
            ...
            None => {
                warn!(
                    &ctx,
                    "access denied: CONFIG map does not contain the init net NS (code bug)"
                );
                return Err(-1);
            }
        }
...
fn send_event_to_userspace(
    call_type: BlockedCall,
    path_buf: *mut [u8; 4096],
    path_len: i64,
) -> Result<(), i32> {
    // log access denied via the ring buffer
    let mut entry = EVENTS.reserve::<Event>(0).ok_or(-1)?;
...
fn try_file_open(ctx: LsmContext) -> Result<i32, i32> {
    ...
    let (path_buf, path_len) = match may_block_from_path(&ctx, path_ptr) {
        ...
        Ok((true, None)) => {
            error!(
                &ctx,
                "codebug: may_block_from_path() returned True but without path and len!"
            );
            return Err(-1);
        }
        Err(v) => return Err(v),
                    ...
                    Some(name_ptr) => {
                        ...
                        let name = unsafe {
                            str::from_utf8_unchecked(
                                bpf_probe_read_kernel_str_bytes(name_ptr, &mut data[..])
                                    .map_err(|_| -1i32)?,
                            )
                        };
                        if name == "padsi" {
...
fn get_ppid(
    task_ptr: *const vmlinux::task_struct,
) -> Result<(i32, *const vmlinux::task_struct), i32> {
    unsafe {
        match bpf_probe_read_kernel(&(*task_ptr).real_parent) {
            Ok(ptask_ptr) => match bpf_probe_read_kernel(&(*ptask_ptr).tgid) {
