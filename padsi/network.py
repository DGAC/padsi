...
def netns_delete(name: str):
    ...
    if netns_exists(name):
        ...
        proc=subprocess.run(["ip", "netns", "del", name], capture_output=True, text=True, check=False)
        if proc.returncode!=0:
            raise NetworkException(f"Could not delete network namespace '{name}': {proc.stderr} (status: {proc.returncode})")

def netns_add(name: str):
    """Add a new network namespace"""
    proc=subprocess.run(["ip", "netns", "add", name], capture_output=True, text=True, check=False)
    if proc.returncode!=0:
        raise NetworkException(f"Could not add network namespaces '{name}': {proc.stderr} (status: {proc.returncode})")

def netns_list() -> list[str]:
    proc = subprocess.run(["ip", "netns", "ls"], capture_output=True, text=True, check=False)
    if proc.returncode!=0:
        raise NetworkException(f"Failed to list named network namespaces: {proc.stderr} (status: {proc.returncode})")
    res = []
...
def list_interfaces(netns: str | None = None) -> dict[str, ipaddress.IPv4Interface]:
    ...
    if proc.returncode!=0:
        raise NetworkException(f"Could not list network interfaces {_with_netns(None, netns)}: {proc.stderr} (status: {proc.returncode})")
...
def interface_check_exists(name: str, netns: str | None = None):
    if not interface_exists(name, netns):
        raise NetworkException(f"Network interface {_with_netns(name, netns)} does not exist")
...
def interface_delete(name: str, netns: str | None = None):
    ...
    if interface_exists(name, netns):
        ...
        proc = subprocess.run(args, capture_output=True, text=True, check=False)
        if proc.returncode!=0:
            raise NetworkException(f"Could not delete network interface {_with_netns(name, netns)}: {proc.stderr}")
...
def interface_is_up(name: str, netns: str | None = None) -> bool | None:
    ...
    if interface_exists(name, netns):
        ...
        proc = subprocess.run(args, capture_output=True, text=True, check=False)
        if proc.returncode != 0:
            raise NetworkException(f"Could not get state of interface {_with_netns(name, netns)}: {proc.stderr} (status: {proc.returncode})")
        data = json.loads(proc.stdout)
        try:
            return "UP" in data[0]["flags"]
        except KeyError as e:
            raise NetworkException(f"CODEBUG: Could not get state of interface {_with_netns(name, netns)}: {e}")
    return None
...
def interface_set_up(name: str, up: bool, netns: str | None = None, mtu:int|None=None):
    ...
    if interface_exists(name, netns):
        ...
        proc = subprocess.run(args, capture_output=True, text=True, check=False)
        if proc.returncode != 0:
            raise NetworkException(f"Could not set state of interface {_with_netns(name, netns)} to {'up' if up else 'down'}: {proc.stderr}")

        if mtu is not None:
            args = _ip_command(netns) + ["link", "set", "dev", name, "mtu", str(mtu)]
            proc=subprocess.run(args, capture_output=True, text=True, check=False)
            if proc.returncode!=0:
                raise NetworkException(f"Could not set MTU of interface {_with_netns(name, netns)} to {mtu}: {proc.stderr}")
...
def interface_attach_to_bridge(name: str, bridge: str, netns: str | None = None):
    ...
    proc = subprocess.run(args, capture_output=True, text=True, check=False)
    if proc.returncode != 0:
        raise NetworkException(f"Could not attach interface {_with_netns(name, netns)} to bridge {_with_netns(bridge, netns)}: {proc.stderr}")
...
def interface_move_to_namespace(name: str, new_netns: str, current_netns: str | None = None):
    ...
    if interface_exists(name, new_netns):
        raise NetworkException(f"Network interface {_with_netns(name, new_netns)} already exists (interface_move_to_namespace)")

    if current_netns:
        # bring interface to the 'init' namespace
        if interface_exists(name):
            raise NetworkException("Network interface '{name}' already exists in the 'init' namespace, we can't use the 'init' namespace as a 'staging' namespace")
        args = _ip_command(current_netns) + ["netns", "exec", current_netns, "ip", "link", "set",
            "netns", "1", "dev", name]
        proc = subprocess.run(args, capture_output=True, text=True, check=False)
        if proc.returncode != 0:
            raise NetworkException(f"Could not move network interface {_with_netns(name, current_netns)} to the 'init' namespace: {proc.stderr} (status: {proc.returncode})")
    ...
    if new_netns:
        ...
        if proc.returncode != 0:
            try:
                ...
                pass
            raise NetworkException(f"Could not move network interface {_with_netns(name, 'init')} to the '{new_netns}' namespace: {proc.stderr} (status: {proc.returncode})")
...
def get_default_interfaces(netns: str|None=None) -> set[str]:
    ...
    if proc.returncode == 0:
        try:
            ...
            raise NetworkException(f"Could not get default route's interface of {_with_netns(netns, 'init')}: {e}")
    raise NetworkException(f"Could not get default route's interface of {_with_netns(netns, 'init')}: {proc.stderr} (status: {proc.returncode})")
...
def addr_exists(addr: ipaddress.IPv4Interface | ipaddress.IPv4Address, netns: str | None = None):
    ...
    proc = subprocess.run(args, capture_output=True, text=True, check=False)
    if proc.returncode != 0:
        raise NetworkException(f"Could not list network addresses in ns {netns if netns else 'init'}")
    iface = None
    for line in proc.stdout.splitlines():
        if re.match(r"^[0-9]+:", line):
            ...
            if isinstance(addr, ipaddress.IPv4Interface) and net == addr and net.ip == addr.ip or \
                isinstance(addr, ipaddress.IPv4Address) and net.ip == addr:
                return iface
    return None
...
def addr_add(iface: str, addr: ipaddress.IPv4Interface, netns: str | None = None):
    ...
    if eiface is not None:
        if eiface == iface:
            ...
            raise NetworkException(f"Can't add address '{addr}' to {_with_netns(iface, netns)}: it is already used by {_with_netns(eiface, netns)}")

    args = _ip_command(netns) + ["addr", "add", str(addr), "dev", iface]
    proc = subprocess.run(args, capture_output=True, text=True, check=False)
    if proc.returncode != 0:
        raise NetworkException(f"Could not add address '{addr}' to {_with_netns(iface, netns)}: {proc.stderr}")
...
def addr_get(iface: str, netns: str | None = None) -> ipaddress.IPv4Interface | None:
    ...
    proc = subprocess.run(args, capture_output=True, text=True, check=False)
    if proc.returncode != 0:
        raise NetworkException(f"Could not get the network addresses in ns {netns if netns else 'init'} of interface '{iface}': {proc.stderr}")

    addr = None
    for line in proc.stdout.splitlines():
        if re.match(r"^[ ]*inet ", line):
            if addr:
                raise NetworkException(f"Interface '{iface}' has more than one IPv4 address in ns {netns if netns else 'init'}")
            (_, addr, *_) = line.split()
...
def veth_add(name: str, netns: str|None, peer_name: str, peer_netns: str):
    ...
    if name == peer_name:
        raise NetworkException("Can't create veth where both endpoints have the same name")
    if interface_exists(name, netns):
        raise NetworkException(f"Network interface {_with_netns(name, netns)} already exists (veth_add)")
    if interface_exists(peer_name, peer_netns):
        raise NetworkException(f"Peer network interface {_with_netns(peer_name, peer_netns)} already exists (veth_add)")
    args = _ip_command(netns) + ["link", "add", name, "type", "veth", "peer", "name", peer_name]
    proc = subprocess.run(args, capture_output=True, text=True, check=False)
    if proc.returncode != 0:
        raise NetworkException(f"Could not create veth interfaces {_with_netns(name, netns)} <-> {_with_netns(peer_name, peer_netns)}: {proc.stderr}")
    ...
    if netns != peer_netns:
        ...
        proc = subprocess.run(args, capture_output=True, text=True, check=False)
        if proc.returncode != 0:
            raise NetworkException(f"Could not move veth interface {_with_netns(peer_name, netns)} to {_with_netns(peer_name, peer_netns)}: {proc.stderr}")
...
def bridge_add(name: str, addr: ipaddress.IPv4Interface, netns: str | None = None):
    ...
    if interface_exists(name, netns):
        raise NetworkException(f"Network interface {_with_netns(name, netns)} already exists (bridge_add)")
    ...
    try:
        ...
        proc = subprocess.run(args, capture_output=True, text=True, check=False)
        if proc.returncode != 0:
            raise NetworkException(f"Could not add network bridge {_with_netns(name, netns)}: {proc.stderr}")

        # configure addr
        args = _ip_command(netns) + ["addr", "add", str(addr), "dev", name]
        proc = subprocess.run(args, capture_output=True, text=True, check=False)
        if proc.returncode != 0:
            raise NetworkException(f"Could not set cidr of bridge {_with_netns(name, netns)}: {proc.stderr}")
...
def tap_add(name: str, addr: ipaddress.IPv4Interface, netns: str | None = None, user: str | None = None):
    ...
    if interface_exists(name, netns):
        raise NetworkException(f"Network interface {_with_netns(name, netns)} already exists (tap_add)")
    try:
        # create bridge
        ...
        proc = subprocess.run(args, capture_output=True, text=True, check=False)
        if proc.returncode != 0:
            raise NetworkException(f"Could not add tap interface {_with_netns(name, netns)}: {proc.stderr}")

        # configure addr
        args = _ip_command(netns) + ["addr", "add", str(addr), "dev", name]
        proc = subprocess.run(args, capture_output=True, text=True, check=False)
        if proc.returncode != 0:
            raise NetworkException(f"Could not set cidr of tap {_with_netns(name, netns)}: {proc.stderr}")
...
def route_add_default(iface: str, gw_addr: ipaddress.IPv4Address | None, netns: str | None = None):
    ...
    if gw_addr is None:
        ...
        args = _ip_command(netns) + ["route", "add", "default", "via", str(gw_addr), "dev", iface]
    proc = subprocess.run(args, capture_output=True, text=True, check=False)
    if proc.returncode != 0:
        raise NetworkException(f"Could not add default via {_with_netns(iface, netns)}: {proc.stderr}")
