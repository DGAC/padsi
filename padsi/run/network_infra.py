...
def network_infra_cleanup(
    ...
    for item in serialized_components:
        ...
        try:
            ...
            syslog.syslog(syslog.LOG_ERR, f"{syslog_prefix}: cleanup failed to stop component {component.name if component is not None else item}: {e}")
    ...
    try:
        ...
        syslog.syslog(syslog.LOG_ERR, f"{syslog_prefix}: cleanup failed to remove masquerade: {e}")
...
def network_infra_dnat_incoming(
    ...
    try:
        ...
        syslog.syslog(syslog.LOG_ERR, f"{syslog_prefix}: could not DNAT incoming traffic to {endpoint}: {e}")
        raise
...
def network_infra_attach_zone_apps(
    ...
    try:
        ...
        syslog.syslog(syslog.LOG_ERR, f"{syslog_prefix}: could not attach zone apps: {e}")
        raise
